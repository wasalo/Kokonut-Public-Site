...
flowchart TD
    A["Proposal passes"]
    B["Grace period opens"]
    C{"Members who disagree?"}
    D["Rage-quit
    → receive proportional treasury share
    → tokens burned"]
    E["Members who agree
    → wait"]
    F{"Token retention ≥ 66%?"}
    G["Proposal executes ✅"]
    H["Proposal does NOT execute ❌"]
...
Proposal passes
      ...
      ▼
Grace period opens
  ├── Members who disagree: rage-quit → receive proportional treasury share → tokens burned
  └── Members who agree: wait
      │
      ▼
Grace period closes
  └── Proposal executes (if token retention threshold ≥ 66% remains)
```
