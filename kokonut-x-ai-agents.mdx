...
flowchart TD
    ...
    POST /mrv · POST /harvests
    POST /attestations · GET /impact"]

    subgraph BASE["BASE — Chain ID 8453"]
        direction LR
        ERC["ERC-8004 Agent Registry"]
        ESC["Escrow · x402 settlement"]
        EAS["EAS attestations"]
        KAR["Karma GAP reputation"]
    end

    MRV -->|"HTTP + x402"| API
    HA -->|"HTTP + x402"| API
    ISA -->|"HTTP + x402"| API
    API -->|"EAS attestation events"| BASE
```
