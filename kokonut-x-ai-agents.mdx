...
flowchart TD
    subgraph OS["Runtime"]
        ...
        MRV["MRV Agent"]
        HA["Harvest Agent"]
        ISA["Impact Scoring Agent"]
    end

    API["Kokonut Farm Registry API\nMRV · Harvests · Attestations · Impact"]

    subgraph ETHEREUM["Ethereum Network — Chain ID 1"]
        direction LR
        ID["Agent Identity\nERC-8004 Registry"]
        PAY["Payments\nx402 Settlement + Escrow"]
        ATT["Verification\nEAS Attestations"]
        REP["Reputation\nKarma GAP"]
        DIS["Disputes\nArbitration"]
    end

    MRV -->|"submits field observations"| API
    HA -->|"submits harvest records"| API
    ISA -->|"requests verified impact data"| API

    API -->|"checks agent identity"| ID
    API -->|"requires payment"| PAY
    API -->|"publishes verified events"| ATT
    API -->|"updates project reputation"| REP
    API -->|"escalates disputes"| DIS

    PAY -->|"payment confirmed"| API
    ATT -->|"impact evidence available"| ISA

    style MRV fill:#d1fae5,stroke:#10b981,color:#065f46
    style HA fill:#d1fae5,stroke:#10b981,color:#065f46
    style ISA fill:#d1fae5,stroke:#10b981,color:#065f46
    style API fill:#009F4D,stroke:#009F4D,color:#ffffff
    style ID fill:#dbeafe,stroke:#3b82f6,color:#1e40af
    style PAY fill:#fef3c7,stroke:#f59e0b,color:#92400e
    style ATT fill:#ecfdf5,stroke:#10b981,color:#065f46
    style REP fill:#ede9fe,stroke:#8b5cf6,color:#4c1d95
    style DIS fill:#fee2e2,stroke:#ef4444,color:#991b1b
```
...
sequenceDiagram
    ...
    participant FarmRegistryAPI as Farm Registry API
    participant Base as Base Network

    Agent->>FarmRegistryAPI: GET /farms/adelphi/health
    FarmRegistryAPI-->>Agent: 402 Payment Required
    Agent->>Base: Settle 5 USDC on Base
    Base-->>Agent: Payment confirmed on-chain
    Agent->>FarmRegistryAPI: GET /farms/adelphi/health with X-PAYMENT header
    FarmRegistryAPI->>Base: Verify payment on-chain
    Base-->>FarmRegistryAPI: Payment valid
    FarmRegistryAPI-->>Agent: Return health report
```
...
<CardGroup cols={2}>
  ...
  <Card title="Glossary" icon="book-open" href="ecosystem-wiki/glossary">
